...
rd_kafka_topic_t * createTopic(const std::string & topic, rd_kafka_t * kf_producer) {
...
void destroyProducer(rd_kafka_t * kf_producer) {
...
int sendMessage(std::string & message, rd_kafka_topic_t * kf_topic) {
    ...
    char * msg_payload = new char [message.length()];
    ...
    return rd_kafka_produce(kf_topic, kf_partition,
...
int main(void) {
    ...
    rd_kafka_t * kf_producer = configureProducer();

    // Now we have our producer. We need to create a topic so that we
    // can send messages.
    rd_kafka_topic_t * kf_topic = createTopic("demo-default-topic", kf_producer);
