...
int main(void) {
    ...
    rd_kafka_t * kf_producer = rd_kafka_new(RD_KAFKA_PRODUCER, kf_conf, errstr,
            512);
    ...
    char kf_default_topic[] = "demo-default-topic";
    rd_kafka_topic_t * kf_topic = rd_kafka_topic_new(kf_producer,
            kf_default_topic, nullptr);
    ...
    int kf_part_msg_flags = RD_KAFKA_MSG_F_COPY;

    char msg_payload[] = "this is a sample message";
    size_t msg_payload_size = strlen(msg_payload);

    // Let's send a message
    // No keys - I don't know how to use them right now - sorry!
    kf_generic_result = rd_kafka_produce(kf_topic, kf_partition,
            kf_part_msg_flags, msg_payload, msg_payload_size, nullptr, 0,
            nullptr);
